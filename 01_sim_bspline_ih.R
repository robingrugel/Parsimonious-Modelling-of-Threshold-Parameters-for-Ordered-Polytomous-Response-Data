...
# fit specifics
Bdegree <- 3 
Bknots <- floor(sqrt(p_list[[p]]$sim_K))
...
for(i in 1:nsim) {
  ...
  alt_knot <- unname(rbind(rep(1, p_list[[p]]$sim_I),
                           apply(sim_run, 2,
                                 function(x) {
                                   ydens <- density(x, adjust = 1.5, from = 0, to = p_list[[p]]$sim_K)
                                   ydens$x[sapply((1:Bknots) / (Bknots + 2), function(q) which(cumsum(ydens$y) / sum(ydens$y) >= q)[1])]
                                 }),
                           rep(p_list[[p]]$sim_K+1, p_list[[p]]$sim_I)))
  
  all_knots <- array(dim = c(p_list[[p]]$sim_I, p_list[[p]]$sim_K-1 ,length(c(0, (1:Bknots) / (Bknots + 1), 1)) + Bdegree))
